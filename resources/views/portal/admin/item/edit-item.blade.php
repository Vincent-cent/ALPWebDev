                    ...
                    <form action="{{ route('admin.items.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <select class="form-select @error('game_id') is-invalid @enderror" 
                                ...
                                @foreach($games as $game)
                                    <option value="{{ $game->id }}" 
                                        {{ old('game_id', $item->game_id) == $game->id ? 'selected' : '' }}>
                                        {{ $game->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('item_id') is-invalid @enderror" 
                                   id="item_id" name="item_id" value="{{ old('item_id', $item->item_id) }}" placeholder="e.g., DIAMOND_100">
                            ...
                            <select class="form-select @error('tipe_item_id') is-invalid @enderror" 
                                ...
                                @foreach($tipoItems as $tipe)
                                    <option value="{{ $tipe->id }}" 
                                        {{ old('tipe_item_id', $item->tipe_item_id) == $tipe->id ? 'selected' : '' }}>
                                        {{ $tipe->nama }}
                                    </option>
                                @endforeach
                            </select>
