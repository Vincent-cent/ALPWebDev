    ...
    <main class="admin-main-content">
        <div class="container-fluid py-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h1>Edit Banner: {{ $bannerPromo->name }}</h1>
            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.banner-promos.update', $bannerPromo) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                           id="name" name="name" value="{{ old('name', $bannerPromo->name) }}" required>
                                    @error('name')
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control @error('order') is-invalid @enderror" 
                                           id="order" name="order" value="{{ old('order', $bannerPromo->order) }}" 
                                           min="0" required>
                                    @error('order')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control @error('game_id') is-invalid @enderror" 
                                        ...
                                        @foreach($games ?? [] as $game)
                                            <option value="{{ $game->id }}" {{ old('game_id', $bannerPromo->game_id) == $game->id ? 'selected' : '' }}>
                                                {{ $game->name }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="mb-2">
                                <img src="{{ asset('storage/' . $bannerPromo->image) }}" 
                                     alt="{{ $bannerPromo->name }}" 
                                     style="max-width: 300px; max-height: 150px; object-fit: cover; border-radius: 4px;">
                            </div>
                            <small class="text-muted">{{ $bannerPromo->image }}</small>
                        </div>
                        @endif
